import { keepAmountsTogether, type Block } from '../data/guides';
              ...
              <ul key={i} className="mt-4 space-y-2.5">
                {b.items.map((it, j) => (
                  <li key={j} className="flex gap-3 text-ink-700 leading-relaxed">
                    ...
                    <span>{keepAmountsTogether(it)}</span>
                  </li>
                ))}
              </ul>
            );
              ...
              <ol key={i} className="mt-4 space-y-2.5">
                {b.items.map((it, j) => (
                  <li key={j} className="flex gap-3 text-ink-700 leading-relaxed">
                    ...
                    <span className="pt-0.5">{keepAmountsTogether(it)}</span>
                  </li>
                ))}
              </ol>
            );
