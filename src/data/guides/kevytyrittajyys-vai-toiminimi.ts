...
export const kevytyrittajyysVaiToiminimi: Guide = {
  ...
  content: {
      ...
      body: [
        ...
        { t: 'note', x: 'YEL-velvollisuus ei poistu kevytyrittäjänä: jos arvioitu YEL-työtulosi ylittää rajan, YEL on pakollinen myös laskutuspalvelun käyttäjälle. ALV-velvollisuus alkaa, kun liikevaihto ylittää 20 000 € kalenterivuodessa (vero.fi, 2026).' },
    ...
    ru: {
      ...
      lead: 'Когда вы начинаете выставлять счета, варианты часто — работа через сервис выставления счетов (kevytyrittäjyys) или собственный toiminimi. Объясняем отличия и кому что подходит.',
      body: [
        { t: 'h2', x: 'Что такое kevytyrittäjyys?' },
        { t: 'p', x: 'Пользователь сервиса выставления счетов выставляет счета через этот сервис без собственной фирмы и Y-tunnus. Сервис ведёт выставление счетов, удержание налога и отчёты в органы и берёт за это плату (часто несколько процентов со счёта). Kevytyrittäjyys — это не отдельная форма бизнеса, а способ выставлять счета.' },
        { t: 'h2', x: 'Что такое toiminimi?' },
        { t: 'p', x: 'Toiminimi — собственная фирма с Y-tunnus. Вы получаете полные вычеты, ведёте (или передаёте на аутсорс) учёт и НДС (ALV) сами, у вас больше контроля и часто более выгодное налогообложение. Ответственности немного больше.' },
        { t: 'h2', x: 'Ключевые отличия' },
        { t: 'ul', items: [
          'Регистрация: пользователь сервиса ничего не открывает; toiminimi регистрируется (заявление о регистрации в PRH 75 €).',
          'Расходы: пользователь сервиса платит комиссию с каждого счёта; у toiminimi расходы более постоянные (напр. бухгалтерия).',
          'Вычеты: у toiminimi шире; у пользователя сервиса ограниченнее.',
          'ALV: toiminimi ведёт сам (при обороте свыше 20 000 € в календарный год); сервис может вести ALV за вас.',
          'YEL: касается обоих, если доход YEL превышает 9 423,09 € (2026).',
        ] },
        { t: 'note', x: 'Обязанность YEL не исчезает у пользователя сервиса выставления счетов: если расчётный доход YEL (YEL-työtulo) превышает порог, YEL обязателен и для него. Обязанность по НДС (ALV) наступает, когда оборот превышает 20 000 € за календарный год (vero.fi, 2026).' },
      ...
      faq: [
        ...
        { q: 'Плачу ли я YEL как пользователь сервиса выставления счетов?', a: 'Да, если расчётный доход YEL (YEL-työtulo) превышает 9 423,09 € (2026). YEL обязателен и для пользователя сервиса.' },
        { q: 'Что дешевле?', a: 'Зависит от объёма счетов. Для случайных счетов обычно дешевле работа через сервис выставления счетов; для регулярных и растущих toiminimi часто выгоднее.' },
        { q: 'Можно ли перейти с работы через сервис выставления счетов на toiminimi?', a: 'Да, и это распространено при росте. Поможем с открытием toiminimi.' },
      ],
    },
    en: {
      ...
      body: [
        ...
        { t: 'ul', items: [
          ...
          'VAT: a toiminimi handles it itself (over €20,000 turnover in a calendar year); an invoicing service can handle VAT for you.',
          'YEL: applies to both if work income exceeds €9,423.09 (2026).',
        ] },
        { t: 'note', x: 'The YEL obligation doesn’t disappear as a light entrepreneur: if your estimated YEL work income exceeds the limit, YEL is mandatory for an invoicing-service user too. VAT registration is required once turnover exceeds €20,000 in a calendar year (vero.fi, 2026).' },
    ...
    et: {
      ...
      body: [
        ...
        { t: 'p', x: 'Toiminimi on oma ettevõte Y-tunnusega. Saad täisvähendused, korraldad (või tellid) raamatupidamise ja käibemaksu (ALV) ise ning sul on rohkem kontrolli ja sageli maksutõhusam olukord. Vastutust on veidi rohkem.' },
        ...
        { t: 'ul', items: [
          ...
          'ALV: toiminimi korraldab ise (üle 20 000 € käibe puhul kalendriaastas); arveldusteenus võib ALV sinu eest korraldada.',
          'YEL: puudutab mõlemat, kui töötulu ületab 9 423,09 € (2026).',
        ] },
        { t: 'note', x: 'YEL-kohustus ei kao kergettevõtjana: kui hinnanguline YEL-töötulu ületab piiri, on YEL kohustuslik ka arveldusteenuse kasutajale. Käibemaksu (ALV) kohustus tekib, kui käive ületab 20 000 € kalendriaastas (vero.fi, 2026).' },
    ...
    uk: {
      ...
      body: [
        ...
        { t: 'p', x: 'Toiminimi — власна фірма з Y-tunnus. Ви отримуєте повні вирахування, ведете (або передаєте на аутсорс) облік і ПДВ (ALV) самі, у вас більше контролю і часто вигідніше оподаткування. Відповідальності трохи більше.' },
        ...
        { t: 'ul', items: [
          ...
          'ALV: toiminimi веде сам (при обороті понад 20 000 € за календарний рік); сервіс може вести ALV за вас.',
          'YEL: стосується обох, якщо дохід YEL перевищує 9 423,09 € (2026).',
        ] },
        { t: 'note', x: 'Обовʼязок YEL не зникає в користувача сервісу виставлення рахунків: якщо розрахунковий дохід YEL (YEL-työtulo) перевищує поріг, YEL обовʼязковий і для нього. Обовʼязок з ПДВ (ALV) виникає, коли оборот перевищує 20 000 € за календарний рік (vero.fi, 2026).' },
